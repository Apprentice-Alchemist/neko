...
inline value atomic_exchange(value *loc, value v) {
  return (value)InterlockedExchange((void **)loc, (void *)value);
}
inline value atomic_compare_exchange_strong(value *loc, value expected,
                                            value desired){
    return (value)InterlockedCompareExchangePointer((void **)loc,
                                                    (void *)expected,
                                                    (void *)desired)}
#else
#define _Atomic
...
inline bool atomic_compare_exchange_strong(value *loc, value *expected,
                                           value desired) {
  return __atomic_compare_exchange(loc, expected, &desired, false,
...
static value neko_atomic_compare_exchange(value loc, value expected,
  ...
  while (val_compare(orig, expected) == 0) {
    if (atomic_compare_exchange_strong(val_atomic(loc), &orig, desired)) {
      break;
    } else {
      continue;
    }
...
static value neko_atomic_fetch_update(value loc, value fun) {
  ...
  while (true) {
    ...
    if (atomic_compare_exchange_strong(val_atomic(loc), &orig, new)) {
      return orig;
    } else {
      continue;
    }
